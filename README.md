...
- **Workday Custom Dropdown Handler & Diacritic Normalization**: Navigates React-based select widgets (`[data-automation-id="selectWidget"]`, `[aria-haspopup="listbox"]`) via native click events, 600ms DOM listbox render delays, 300ms menu close delays, and diacritic normalization (`.normalize("NFD").replace(/[\u0300-\u036f]/g, "")`) matching (e.g. `"Karnataka"` ➔ `"Karnātaka"`).
